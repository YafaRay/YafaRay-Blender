...
class YAF_PT_preview_controls(MaterialButtonsPanel, Panel):
    ...
    def draw(self, context):
        if context.scene.yafaray.preview.enable:
            ...
            split = layout.split() 
            col = split.column()
            if context.scene.yafaray.preview.camDynRotRunning:
                col.operator("preview.dynamiccamrot", text='Dynamic camRot running - press ESC or Right click to exit', icon="MANIPUL")
            else:
                col.operator("preview.dynamiccamrot", text='Start Dynamic camRot')
                col = split.column()
