...
def get_render_coords(scene_blender):
    ...
    if not cam_data:
        ...
        # camera = scene_blender.objects.camera.getData()
...
def define_camera(camera, res_x, res_y, res_percentage, use_view_to_render, view_matrix):
    ...
    film_yafaray.defineCamera(param_map)
...
def export_render_settings(depsgraph, render_path, render_filename):
    logger.printVerbose("Exporting Render Settings")
    ...
    export_aa(scene_blender, param_map)
...
def set_logging_and_badge_settings(scene_blender, param_map):
    logger.printVerbose("Exporting Logging and Badge settings")
...
def define_layers(depsgraph):
    logger.printVerbose("Exporting Render Passes settings")
    scene_blender = scene_from_depsgraph(depsgraph)
...
def define_image_output(output_name, fp, scene_blender, bl_render, color_space, gamma, alpha_premultiply):
    output_file, output, file_type = decide_output_file_name(fp, scene_blender.img_output)
    param_map = libyafaray4_bindings.ParamMap()
    ...
    # film_yafaray.setLoggingAndBadgeSettings(scene_yafaray, scene_blender)
    co = film_yafaray.createOutput(output_name, param_map)
...
def update_blender_result(x, y, w, h, view_name, tiles, callback_name):
    ...
    if scene_blender.render.use_multiview:
        blender_result_buffers = begin_result(x, y, w, h, "", view_name)
    else:
        blender_result_buffers = begin_result(x, y, w, h)
    for tile in tiles:
        ...
        try:
            ...
            traceback.print_exc()
    end_result(blender_result_buffers)
...
def highlight_callback(*args):
    ...
    if view_name == "":  # In case we use Render 3D viewport with Views enabled, it will copy the result to all views
        for view in scene_blender.render.views:
...
def render(film_yafaray, integrator_yafaray, scene_blender, scene_yafaray, logger, is_preview, progress_callback):
    film_yafaray.setFlushAreaCallback(flush_area_callback)
    film_yafaray.setFlushCallback(flush_callback)
    film_yafaray.setHighlightAreaCallback(highlight_callback)
    ...
    scene_modified_flags = scene_yafaray.checkAndClearModifiedFlags()
    scene_yafaray.preprocess(render_control, scene_modified_flags)
    integrator_yafaray.preprocess(render_monitor, render_control, scene_yafaray)
    integrator_yafaray.render(render_control, render_monitor, film_yafaray)
    return  # FIXME!!!
    t = threading.Thread(target=yaf_integrator, args=(scene_yafaray, progressCallback,))
    t.start()

    while t.is_alive() and not test_break():
        time.sleep(0.2)

    if t.is_alive():
        update_stats("",
                     "Aborting, please wait for all pending tasks to complete (progress in console log)...")
        scene_yafaray.cancelRendering()
        t.join()
