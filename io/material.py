...
class MaterialControl:
    ...
    def get_used_textures(material):
        ...
        #                     "TEX_IMAGE"):  # FIXME BLENDER >= v2.80 TEX_ but later getting .texture attribute does not work
    ...
    def write_glass_shader(self, mat, scene, rough):
        ...
        for mtex in used_textures:
            ...
            if self.write_tex_layer(param_map_list, lname, mappername, mcol_root, mtex, mtex.use_map_mirror, mir_col,
                                    mtex.mirror_factor):
                used = True
                mcol_root = lname
            lname = "bump_layer%x" % i
            if self.write_tex_layer(param_map_list, lname, mappername, bump_root, mtex, mtex.use_map_normal, [0],
                                    mtex.normal_factor):
                used = True
                bump_root = lname
            lname = "filter_color_layer%x" % i
            if self.write_tex_layer(param_map_list, lname, mappername, filter_color_root, mtex,
                                    mtex.use_map_color_reflection, filt_col, mtex.reflection_color_factor):
                used = True
                filter_color_root = lname
            lname = "IOR_layer%x" % i
            if self.write_tex_layer(param_map_list, lname, mappername, ior_root, mtex, mtex.use_map_warp, [0],
                                    mtex.warp_factor):
                used = True
                ior_root = lname
            lname = "wireframe_layer%x" % i
            if self.write_tex_layer(param_map_list, lname, mappername, wireframe_root, mtex, mtex.use_map_displacement,
                                    [0], mtex.displacement_factor):
                used = True
                wireframe_root = lname
            lname = "roughness_layer%x" % i
            if self.write_tex_layer(param_map_list, lname, mappername, roughness_root, mtex, mtex.use_map_hardness, [0],
        ...
        return self.scene_yafaray.create_material(mat.name, param_map, param_map_list)
    ...
    def write_glossy_shader(self, mat, scene, coated):  # mat : instance of material class
        ...
        for mtex in used_textures:
            ...
            if self.write_tex_layer(param_map_list, lname, mappername, diff_root, mtex, mtex.use_map_color_diffuse,
                                    diffuse_color, mtex.diffuse_color_factor):
                used = True
                diff_root = lname
            lname = "gloss_layer%x" % i
            if self.write_tex_layer(param_map_list, lname, mappername, gloss_root, mtex, mtex.use_map_color_spec, color,
                                    mtex.specular_color_factor):
                used = True
                gloss_root = lname
            lname = "glossref_layer%x" % i
            if self.write_tex_layer(param_map_list, lname, mappername, gl_ref_root, mtex, mtex.use_map_specular,
            ...
            if self.write_tex_layer(param_map_list, lname, mappername, sigma_oren_root, mtex, mtex.use_map_hardness,
                                    [0], mtex.hardness_factor):
                used = True
                sigma_oren_root = lname
            lname = "exponent_layer%x" % i
            if self.write_tex_layer(param_map_list, lname, mappername, exponent_root, mtex, mtex.use_map_ambient, [0],
                                    mtex.ambient_factor):
                used = True
                exponent_root = lname
            lname = "IOR_layer%x" % i
            ...
            if self.write_tex_layer(param_map_list, lname, mappername, diff_reflect_root, mtex, mtex.use_map_diffuse,
            ...
            if coated:
                lname = "mircol_layer%x" % i
                if self.write_tex_layer(param_map_list, lname, mappername, mcol_root, mtex, mtex.use_map_mirror,
                                        mir_col, mtex.mirror_factor):
                    used = True
                    mcol_root = lname
                lname = "mirr_layer%x" % i
                if self.write_tex_layer(param_map_list, lname, mappername, mirror_root, mtex, mtex.use_map_raymir,
    ...
    def write_shiny_diffuse_shader(self, mat, scene):
        ...
        for mtex in used_textures:
            ...
            if mat.clay_exclude or scene.gs_clay_render_keep_transparency or not scene.gs_clay_render:
                ...
                if self.write_tex_layer(param_map_list, lname, mappername, transp_root, mtex, mtex.use_map_alpha,
            ...
            if mat.clay_exclude or scene.gs_clay_render_keep_transparency or not scene.gs_clay_render:
                ...
                if self.write_tex_layer(param_map_list, lname, mappername, transl_root, mtex, mtex.use_map_translucency,
            ...
            if mat.clay_exclude or not scene.gs_clay_render:
                lname = "diff_refl_layer%x" % i
                if self.write_tex_layer(param_map_list, lname, mappername, diff_reflect_root, mtex,
            ...
            if mat.clay_exclude or not scene.gs_clay_render:
                lname = "wireframe_layer%x" % i
                if self.write_tex_layer(param_map_list, lname, mappername, wireframe_root, mtex,
        ...
        if scene.gs_clay_render and not mat.clay_exclude:
            if scene.gs_clay_oren_nayar:
                param_map.set_string("diffuse_brdf", "oren_nayar")
                param_map.set_float("sigma", scene.gs_clay_sigma)
    ...
    def write_blend_shader(self, mat, scene):
        ...
        self.logger.print_info("Exporter: Blend material with: [" + mat.material1name + "] [" + mat.material2name + "]")
        ...
        for mtex in used_textures:
            ...
            if mat.clay_exclude or not scene.gs_clay_render:
                lname = "diff_layer%x" % i
                if self.write_tex_layer(param_map_list, lname, mappername, diff_root, mtex, mtex.use_map_diffuse, [0],
                                        mtex.diffuse_factor):
                    used = True
                    diff_root = lname
    ...
    def write_null_mat(self, mat):
        param_map_list = libyafaray4_bindings.ParamMapList()
        param_map = libyafaray4_bindings.ParamMap()
        param_map.set_string("type", "null")
        return self.scene_yafaray.create_material(mat.name, param_map, param_map_list)

    def write_material(self, material_blender, scene_blender):
        self.logger.print_info("Exporter: Creating Material: \"" + material_blender.name + "\"")
        if material_blender.name == "y_null":
            self.write_null_mat(material_blender)
            ...
            self.write_shiny_diffuse_shader(material_blender, scene_blender)
        elif material_blender.mat_type == "glass":
            self.write_glass_shader(material_blender, scene_blender, False)
        elif material_blender.mat_type == "rough_glass":
            self.write_glass_shader(material_blender, scene_blender, True)
        elif material_blender.mat_type == "glossy":
            self.write_glossy_shader(material_blender, scene_blender, False)
        elif material_blender.mat_type == "coated_glossy":
            self.write_glossy_shader(material_blender, scene_blender, True)
        elif material_blender.mat_type == "shinydiffusemat":
            self.write_shiny_diffuse_shader(material_blender, scene_blender)
        elif material_blender.mat_type == "blend":
            self.write_blend_shader(material_blender, scene_blender)  # FIXME: in the new Clay render two limitations:
    ...
    def handle_blend_mat(self, mat):
        ...
        if mat1.mat_type == 'blend':
            material_blender_error = self.handle_blend_mat(mat1)
            if material_blender_error:
                return

        elif mat1 not in self.material_set:
            self.material_set.add(mat1)
            self.write_material(mat1, self.scene_blender)

        if mat2.mat_type == 'blend':
            material_blender_error = self.handle_blend_mat(mat2)
            if material_blender_error:
                return

        elif mat2 not in self.material_set:
            self.material_set.add(mat2)
            self.write_material(mat2, self.scene_blender)

        if mat not in self.material_set:
            self.material_set.add(mat)
            self.write_material(mat, self.scene_blender)

    def export_material(self, material_blender):
        if material_blender:
            if material_blender.mat_type == 'blend':
                ...
                self.handle_blend_mat(material_blender)
            else:
                self.material_set.add(material_blender)
                self.write_material(material_blender, self.scene_blender)
